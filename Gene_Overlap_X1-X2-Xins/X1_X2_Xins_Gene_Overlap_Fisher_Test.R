...
## and transcripts enriched in X1-, X2-, and Xins subpopulations
##
...
keep <- !is.na(edgeR_zeng$logFC_X1_vs_Xins)
zeng_X_detected <- edgeR_zeng[keep,] 
NROW(zeng_X_detected) # 14403

# now use Venn Diagram to determine overlap (transcriptome size)
my_venn <- list(apob_detected=apob_detected$geneID, zeng_X_gates_detected=zeng_X_detected$geneID)
...
# therefore for apob vs Zeng X gates, transcriptome size = 13954:
ts.zeng <- 13954


##############################################################################################
##
## import X1, X2, Xins and apob-M, apob-S lists
##
##############################################################################################
...
# make blank list
Zeng_X_gates_LIST <- list()
# now fill first element with X1:
Zeng_X_gates_LIST[1] <- read.delim("20210521_X1_Sig_list.txt", header=FALSE, as.is = TRUE) # as.is = TRUE converts to character
# then X2
Zeng_X_gates_LIST[2] <- read.delim("20210521_X2_Sig_list.txt", header=FALSE, as.is = TRUE)
# then Xins
Zeng_X_gates_LIST[3] <- read.delim("20210521_Xins_Sig_list.txt", header=FALSE, as.is = TRUE)
# and rename
names(Zeng_X_gates_LIST) <- c("X1_Sig", "X2_Sig", "Xins_Sig")
# print (optional, show list)
print(Zeng_X_gates_LIST)
# print (optional, show list)
print(Zeng_X_gates_LIST$X1_Sig)
# print (optional, show list)
print(Zeng_X_gates_LIST[1])
...
sapply(Zeng_X_gates_LIST, length)

# output:
# X1_Sig   X2_Sig Xins_Sig 
# 3572      819     3959 
...
ts.zeng <- 13954 # from above

# run GeneOverlap test
gom.obj <- newGOM(apob_dysreg_LIST, Zeng_X_gates_LIST,  
                  ts.zeng)
...
# write pval matrix to data frame and then to file
df <- as.data.frame(getMatrix(gom.obj, name="pval"))
df <- cbind(Zeng_X_gate = rownames(df), df)
rownames(df) <- NULL
write.table(df, "20210527_GeneOverlap_Matrix_apob_vs_ZENG_X_GATES.xls", sep="\t", row.names=FALSE)
